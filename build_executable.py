...
def collect_data_files(base_dir):
    ...
    if assets_dir.exists():
        for asset_file in assets_dir.rglob("*"):
            if asset_file.is_file() and asset_file.suffix in ['.ico', '.png', '.jpg', '.jpeg', '.svg']:
...
def install_pyinstaller_if_needed():
    ...
    try:
        subprocess.run(["pipenv", "run", "pyinstaller", "--version"], 
...
def main():
    ...
    parser.add_argument("--dev", action="store_true", 
                       help="Build in development mode (with console)")
