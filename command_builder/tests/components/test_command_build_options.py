...
def test_build_command_with_flag_checked(qapp):
    ...
    command = Command(
        ...
        arguments=[
            ...
            Argument(
                code="DEBUG", name="Debug", type="flag", required=0, value="--debug"
            ),
            Argument(
                code="VERBOSE",
                name="Verbose",
...
def test_build_command_with_option_checked_and_filled(qapp):
    ...
    command = Command(
        ...
        arguments=[
            Argument(code="INPUT", name="Input", type="file", required=1),
            Argument(
                code="LOG_LEVEL", name="Log Level", type="valued_option", required=0
            ),
        ],
    )
...
def test_build_command_multiple_options(qapp):
    ...
    command = Command(
        ...
        arguments=[
            ...
            Argument(
                code="LOG_FILE", name="Log File", type="valued_option", required=0
            ),
...
def test_build_command_valued_option_removes_cli_flag(qapp):
    ...
    command = Command(
        ...
        arguments=[
            Argument(code="INPUT", name="Input", type="file", required=1),
            Argument(
                code="PROJECT_NAME", name="Project", type="valued_option", required=0
            ),
            Argument(
                code="LOG_FILE", name="Log File", type="valued_option", required=0
            ),
...
def test_build_command_optional_positional_argument_removed(qapp):
    ...
    command = Command(
        ...
        arguments=[
            ...
            Argument(
                code="TXT_DIR", name="Répertoire texte", type="directory", required=1
            ),
            Argument(
                code="IMG_DIR", name="Répertoire images", type="directory", required=1
            ),
            Argument(
                code="TOLERANCE",
                name="Tolérance image",
                type="float",
                required=0,
                default="",
            ),
