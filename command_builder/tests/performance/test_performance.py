...
def large_task():
    ...
    for i in range(100):
        ...
        commands.append(Command(
            ...
            command=f"test_command_{i} " + " ".join(f"{{{arg.code}}}" for arg in arguments),
            arguments=arguments
        ))
    
    return Task(
        name="Large Task",
        description="Task with many commands",
        commands=commands
    )
...
def task_with_many_shared_arguments():
    ...
    for i in range(50):
        ...
        task_arguments.append(TaskArgument(
            code=f"SHARED_{i}",
...
class TestValidationPerformance:
    ...
    def test_validate_large_task_performance(self, large_task):
        ...
        for command in large_task.commands:
            command_values = {
                arg.code: all_values.get(arg.code, "")
                for arg in command.arguments
            }
...
class TestSharedArgumentsPerformance:
    ...
    def test_multiple_apply_shared_arguments_performance(self, task_with_many_shared_arguments):
        ...
        for iteration in range(10):
            shared_values = {
                f"SHARED_{i}": f"value_{i}_{iteration}"
                for i in range(50)
            }
            task.apply_shared_arguments(shared_values)
